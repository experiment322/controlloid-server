...
enum FFB_CTRL {
    ...
    CTRL_STOPALL = 3,    // Stop All EffectsÂ­ Issues a stop on every running effect.
    CTRL_DEVRST = 4,    // Device Resetâ€“ Clears any device paused condition, enables all actuators and clears all effects from memory.
    CTRL_DEVPAUSE = 5,    // Device Pauseâ€“ The all effects on the device are paused at the current time step.
    CTRL_DEVCONT = 6,    // Device Continueâ€“ The all effects that running when the device was paused are restarted from their last time step.
};
...
typedef struct _FFB_EFF_REPORT {
    ...
    BYTE Gain;
    BYTE TrigerBtn;
    BOOL Polar; // How to interpret force direction Polar (0-360Â°) or Cartesian (X,Y)
    union {
        BYTE Direction; // Polar direction: (0x00-0xFF correspond to 0-360Â°)
...
typedef struct _FFB_EFF_PERIOD {
    ...
    LONG Offset;                // Range: â€“10000 - 10000
...
typedef struct _FFB_EFF_COND {
    BYTE EffectBlockIndex;
    BOOL isY;
    LONG CenterPointOffset; // CP Offset:  Range -Â­10000 Â­- 10000
    LONG PosCoeff; // Positive Coefficient: Range -Â­10000 Â­- 10000
    LONG NegCoeff; // Negative Coefficient: Range -Â­10000 Â­- 10000
    DWORD PosSatur; // Positive Saturation: Range 0 â€“ 10000
    DWORD NegSatur; // Negative Saturation: Range 0 â€“ 10000
    LONG DeadBand; // Dead Band: : Range 0 â€“ 1000
...
VJOYINTERFACE_API BOOL    __cdecl
GetNumberExistingVJD(int *n);    // What is the number of vJoy devices currently enabled


/////	vJoy Device properties
VJOYINTERFACE_API int __cdecl GetVJDButtonNumber(UINT rID);    // Get the number of buttons defined in the specified VDJ
VJOYINTERFACE_API int __cdecl
GetVJDDiscPovNumber(UINT rID);    // Get the number of descrete-type POV hats defined in the specified VDJ
VJOYINTERFACE_API int __cdecl
GetVJDContPovNumber(UINT rID);    // Get the number of descrete-type POV hats defined in the specified VDJ
VJOYINTERFACE_API BOOL    __cdecl
GetVJDAxisExist(UINT rID, UINT Axis); // Test if given axis defined in the specified VDJ
VJOYINTERFACE_API BOOL    __cdecl GetVJDAxisMax(UINT rID, UINT Axis,
                                                LONG *Max); // Get logical Maximum value for a given axis defined in the specified VDJ
VJOYINTERFACE_API BOOL    __cdecl GetVJDAxisMin(UINT rID, UINT Axis,
                                                LONG *Min); // Get logical Minimum value for a given axis defined in the specified VDJ
VJOYINTERFACE_API enum VjdStat    __cdecl
GetVJDStatus(UINT rID);            // Get the status of the specified vJoy Device.
// Added in 2.1.6
VJOYINTERFACE_API BOOL    __cdecl
isVJDExists(UINT rID);                    // TRUE if the specified vJoy Device exists
// Added in 2.1.8
VJOYINTERFACE_API int __cdecl
GetOwnerPid(UINT rID);                    // Reurn owner's Process ID if the specified vJoy Device exists
...
VJOYINTERFACE_API BOOL        __cdecl
UpdateVJD(UINT rID, PVOID pData);    // Update the position data of the specified vJoy Device.
...
__declspec(deprecated("** FfbStart function was deprecated - you can remove it from your code **")) \
        VJOYINTERFACE_API BOOL        __cdecl
FfbStart(UINT rID);                  // Start the FFB queues of the specified vJoy Device.
__declspec(deprecated("** FfbStop function was deprecated - you can remove it from your code **")) \
        VJOYINTERFACE_API VOID __cdecl
FfbStop(UINT rID);                  // Stop the FFB queues of the specified vJoy Device.
